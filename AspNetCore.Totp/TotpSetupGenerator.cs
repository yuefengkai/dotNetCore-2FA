...
namespace AspNetCore.Totp
{
    public class TotpSetupGenerator : ITotpSetupGenerator
    {
        ...
        public TotpSetup Generate(string issuer, string accountIdentity, string accountSecretKey, int qrCodeWidth = 300,
            int qrCodeHeight = 300, bool useHttps = true)
        {
            ...
            var provisionUrl = UrlEncoder.Encode(string.Format("otpauth://totp/{0}?secret={1}&issuer={2}",
                accountIdentity, encodedSecretKey, UrlEncoder.Encode(issuer)));
            ...
            {
                QrCodeImage = provisionUrl, //this.GetQrImage(url),
